...
std::vector<long> ThresholdCount::countCapacityMisses(std::vector<long> CacheSizes, isl::pw_qpolynomial Count) {
...
// #ifdef RASTERIZATION
  ...
  applyRasterization(Count.domain());
  // count affine pieces again
  storeAffinePieces();
// #endif
...
void ThresholdCount::storeAffinePieces() {
...
std::vector<long>
ThresholdCount::enumerateNonAffinePoints(piece Piece, std::vector<long> Limits) const {
...
std::vector<long>
ThresholdCount::countAffineDimensions(piece Piece, std::vector<long> Limits) const {
...
std::vector<int> ThresholdCount::findNonAffineDimensions(piece Piece) const {
...
void ThresholdCount::enumerateNonAffineDimensions(piece Piece) {
...
void ThresholdCount::applyEqualization() {
...
void ThresholdCount::applyRasterization(isl::set Domain) {
  Timer::startTimer("applyRasterization");
  // compute the extent of the domain
  std::vector<long> Sizes;
  for (int i = 0; i < Domain.dim(isl::dim::set); ++i) {
    long LowerBound = isl::computeMinimum(Domain, i);
    long UpperBound = isl::computeMaximum(Domain, i);
...
int ThresholdCount::computeExponent(piece Piece) const {
  int Result = 0;
  // check all terms
  auto analyzeTerms = [&](isl::term Term) {
    int Exponent = 0;
...
int ThresholdCount::computeDimensionExponent(int Dimension, piece Piece) const {
  int Result = 0;
  // check all terms
  auto analyzeTerms = [&](isl::term Term) {
    int Exponent = 0;
...
isl::qpolynomial
ThresholdCount::computeReplacement(std::map<int, isl::qpolynomial> Replacements,
                           piece Piece) const {
...
std::vector<int> ThresholdCount::findRasterDimensions(piece Piece) const {
...
std::vector<isl::val> ThresholdCount::computeMultipliers(std::vector<int> Dimensions,
                                                 piece Piece) const {
...
std::vector<piece> ThresholdCount::rasterDimension(int Dimension, isl::val Multiplier,
                                           piece Piece) const {
...
ThresholdCount::findEqualizationCandidates(piece Piece) const {
...
std::vector<long>
ThresholdCount::computeSplits(std::vector<std::tuple<int, long, long>> Candidate,
                      piece Piece) const {
...
std::vector<piece>
ThresholdCount::equalizeCandidate(std::vector<std::tuple<int, long, long>> Candidate,
...
bool ThresholdCount::verifySplit(piece Piece, std::vector<piece> Updates) const {
...
piece ThresholdCount::createPiece(isl::set Domain, isl::qpolynomial Count) const {
...
void ThresholdCount::extractStackDistanceExpression(isl::pw_qpolynomial Polynomial) {
...
bool ThresholdCount::isPieceAffine(piece Piece) const {
...
long ThresholdCount::getPieceSize(piece &Piece) const {
  if (Piece.Size == -1) {
...
isl::pw_aff ThresholdCount::extractAffineExpression(piece Piece) const {
...
isl::aff ThresholdCount::extractAffineExpression(isl::qpolynomial Polynomial,
                                         isl::set Domain,
